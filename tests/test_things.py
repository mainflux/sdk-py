...
def test_create_thing(requests_mock):
    requests_mock.register_uri(
        "POST", url + "/things", headers={"location": "/things/" + thing_id}, status_code=201)
...
def test_create_existing_thing(requests_mock):
    requests_mock.register_uri(
        "POST", url + "/things", headers={"location": "/things/" + thing_id}, status_code=409)
    r = s.things.create(thing=thing, token=token)
...
def test_create_bulk_things(requests_mock):
    requests_mock.register_uri("POST", url + "/things/bulk", json=[
                               thing_id, thing_id1], headers={"location": "/things/" + thing_id}, status_code=201)
...
def test_create_bulk_things_missing_token(requests_mock):
    requests_mock.register_uri("POST", url + "/things/bulk", json=[
                               thing_id, thing_id1], headers={"location": "/things/" + thing_id}, status_code=401)
    r = s.things.create_bulk(things=things, token=token)
    assert r.error.status == 1
...
def test_get_thing(requests_mock):
    requests_mock.register_uri(
        "GET", url + "/things/" + thing_id, json=thing, status_code=200)
...
def test_get_thing_malformed_query(requests_mock):
    requests_mock.register_uri(
        "GET", url + "/things/" + thing_id, json=thing, status_code=400)
    r = s.things.get(thing_id=thing_id, token=token)
    assert r.error.status == 1
...
def test_get_all_things(requests_mock):
    requests_mock.register_uri(
        "GET", url + "/things", json=[thing_id, thing_id1], status_code=200)
...
def test_get_all_thing_does_not_exist(requests_mock):
    requests_mock.register_uri(
        "GET", url + "/things", json=[thing_id, thing_id1], status_code=404)
    r = s.things.get_all(token=token, query_params=params)
    assert r.error.status == 1
...
def test_get_by_channel(requests_mock):
    requests_mock.register_uri("GET", url + "/channels/" + channel_id + "/things", json=channel_id,
                               headers={"Authorization": "/channels/" + channel_id + "/things"}, status_code=200)
    r = s.things.get_by_channel(
        channel_id=channel_id, query_params=params, token=token)
    assert r.error.status == 0
...
def test_get_by_channel_missing_token(requests_mock):
    requests_mock.register_uri("GET", url + "/channels/" + channel_id + "/things", json=channel_id,
                               headers={"Authorization": "/channels/" + channel_id + "/things"}, status_code=401)
    r = s.things.get_by_channel(
        channel_id=channel_id, query_params=params, token=token)
    assert r.error.status == 1
    assert r.error.message == "Missing or invalid access token provided."
...
def test_update_thing(requests_mock):
    requests_mock.register_uri(
        "PUT", url + "/things/" + thing_id, json=json.dumps(thing), status_code=200)
    r = s.things.update(thing_id=thing_id, token=token, thing=thing)
    assert r.error.status == 0


def test_update_thing_bad_json(requests_mock):
    requests_mock.register_uri(
        "PUT", url + "/things/" + thing_id, json=json.dumps(thing), status_code=400)
    r = s.things.update(thing_id=thing_id, token=token, thing=thing)
    assert r.error.status == 1
...
def test_delete_thing(requests_mock):
    requests_mock.register_uri(
        "DELETE", url + "/things/" + thing_id, status_code=204)
    r = s.things.delete(thing_id=thing_id, token=token)
    assert r.error.status == 0


def test_delete_bad_thing_id(requests_mock):
    requests_mock.register_uri(
        "DELETE", url + "/things/" + thing_id, status_code=400)
    r = s.things.delete(thing_id=thing_id, token=token)
    assert r.error.status == 1
...
def test_connect_thing(requests_mock):
    requests_mock.register_uri(
        "PUT", url + "/channels/" + channel_id + "/things/"+thing_id, json=[channel_id, thing_id], status_code=200)
    r = s.things.connect(channel_id=channel_id, thing_id=thing_id, token=token)
    assert r.error.status == 0
...
def test_connect_non_existing_entity(requests_mock):
    requests_mock.register_uri(
        "PUT", url + "/channels/" + channel_id + "/things/"+thing_id, json=[channel_id, thing_id], status_code=404)
    r = s.things.connect(channel_id=channel_id, thing_id=thing_id, token=token)
    assert r.error.status == 1
...
def test_disconnect_thing(requests_mock):
    requests_mock.register_uri(
        "DELETE", url + "/channels/" + channel_id + "/things/" + thing_id, status_code=204)
    r = s.things.disconnect(channel_id=channel_id,
                            thing_id=thing_id, token=token)
    assert r.error.status == 0


def test_disconnect_thing_or_channel_does_not_exist(requests_mock):
    requests_mock.register_uri(
        "DELETE", url + "/channels/" + channel_id + "/things/" + thing_id, status_code=404)
    r = s.things.disconnect(channel_id=channel_id,
                            thing_id=thing_id, token=token)
    assert r.error.status == 1
...
def test_disconnects(requests_mock):
    requests_mock.register_uri("DELETE", url + "/disconnect", status_code=200)
    r = s.things.disconnects(channel_ids=[channel_id], thing_ids=[
                             thing_id, thing_id1], token=token)
    assert r.error.status == 1


def test_disconnects_bad_json(requests_mock):
    requests_mock.register_uri("DELETE", url + "/disconnect", status_code=400)
    r = s.things.disconnects(channel_ids=[channel_id], thing_ids=[
                             thing_id, thing_id1], token=token)
    assert r.error.status == 1
    assert r.error.message == "Failed due to malformed query parameters."
